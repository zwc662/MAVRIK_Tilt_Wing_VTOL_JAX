...
def interpolate(indices: np.ndarray, weights: np.ndarray, interpoloator: JaxNDInterpolator) -> np.ndarray:
    # Compute all corner indices
    corner_indices = indices + interpoloator.corner_offsets
    corner_indices = np.clip(corner_indices, 0, np.array(interpoloator.shape) - 1)

    # Convert n-dimensional indices to flat indices
    flat_corner_indices = np.ravel_multi_index(corner_indices.T, interpoloator.shape)

    # Gather corner values
    corner_values = interpoloator.flat_values[flat_corner_indices]

    # Compute weights for all corners
    corner_weights = np.prod(np.where(interpoloator.corner_offsets, weights, 1 - weights), axis=1)

    # Sum weighted corner contributions
    return np.sum(corner_values * corner_weights)


def interpolate_nd(inputs: np.ndarray, interpoloator: JaxNDInterpolator) -> np.ndarray:
    """
